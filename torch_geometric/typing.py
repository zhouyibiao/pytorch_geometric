...
try:
    ...
    if WITH_SEGMM and 'pytest' in sys.modules and hasattr(torch, 'musa') and torch.musa.is_available():
        ...
        try:
            x = torch.randn(3, 4, device='musa')
            ptr = torch.tensor([0, 2, 3], device='musa')
            weight = torch.randn(2, 4, 4, device='musa')
